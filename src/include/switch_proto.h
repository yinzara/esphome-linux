 ...
 *
 * These functions extend esphome_proto.h with Switch-specific messages.
 */
...
typedef struct {
    ...
    bool state;                 // Field 2: Requested state
...
typedef struct {
    uint32_t key;               // Field 1: Entity key
    bool state;                 // Field 2: Current state
} switch_state_response_t;


/**
 * List Entities Switch Response structure (message ID 17)
...
/**
 * Encode ListEntitiesSwitchResponse to protobuf
 *
 * @param buf Output buffer
 * @param size Buffer size
